...
import CheckpointInitModal from "./checkpoint-init-modal"
...
export default function HeroContent() {
  ...
  // Check if checkpoint count is already set
  useEffect(() => {
    if (!medicineData?.id) return

    const checkProgress = async () => {
      try {
        const res = await fetch(`/api/progress?med=${medicineData.id}`)
        const data = await res.json()

        // Check if checkpoint_count exists AND is greater than 0
        // Only hide modal if checkpoint_count was explicitly set (not the default 3)
        if (data?.checkpoint_count && data.checkpoint_count > 0) {
          setCheckpointCount(data.checkpoint_count)
          setIsInitialized(true)
        } else {
          // Show modal if no checkpoint count set
  ...
  const handleCheckpointConfirm = async (count: number) => {
    if (!medicineData?.id) return

    try {
      // Update checkpoint count in database with snake_case to match Supabase schema
      await fetch("/api/progress", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          medId: medicineData.id,
          progress: {
      ...
      console.error("Failed to save checkpoint count:", err)
    }
  }
    ...
    <>
      ...
      <main className="absolute inset-0 z-20 flex items-center justify-center p-8">
        <div className="fixed right-8 top-1/2 transform -translate-y-1/2">
          {isInitialized && checkpointCount && (
            <ProgressFlow medicineId={medicineData.id} checkpointCount={checkpointCount} />
          )}
        </div>
            ...
            {isInitialized && checkpointCount && (
              <div className="bg-blue-500/20 border border-blue-500/50 rounded-lg p-3 mb-6">
                <p className="text-blue-300 text-sm">
                  <strong>Checkpoints:</strong> {checkpointCount}
                </p>
              </div>
            )} */}
